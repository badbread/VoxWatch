...
For dispatch personas (``police_dispatch``), the
...
# Any .onnx files found on disk that are not in the known list are also
# returned so manually-added models are visible.
#
# The DELETE endpoint only removes files from the downloaded cache directory.
# Builtin voices cannot be deleted (they live in the image layer).
