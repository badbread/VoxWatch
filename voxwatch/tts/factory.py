...
def _build_provider(name: str, config: dict) -> Optional[TTSProvider]:
    ...
    Returns:
        Initialized TTSProvider instance, or None if the provider is
        unavailable (binary missing, SDK not installed, key absent).
    """
    try:
        if name == "piper":
...
def get_provider(config: dict) -> TTSProvider:
    ...
    fallback = _build_provider("espeak", config)
    if fallback is None:
        raise RuntimeError(
...
def get_fallback_chain(config: dict) -> list[TTSProvider]:
    ...
    for name in deduped:
        instance = _build_provider(name, config)
